...
public class LeafNode extends RegressionTreeNode implements Serializable {
	 ...
	 */
	private Regressor regressionModelLearner;
	...
	/**
	 * Constructs empty leaf element with the given regressor as its regression
	 * model.
	 */
	public LeafNode(RegressionTreeNode parent, boolean isRightChild,
	 ...
	 * 
	 * @param item
	 *            example to learn with
	 */
	public void updateBTrees(Data item) {
		n++;
		/*
		 * for (String feature : item.getFeatures()) { Serializable value =
		 * item.get(feature); BTreeNode bTree = bTrees.get(feature); if (bTree
		 * != null) { if (item.get(feature) instanceof Number) {
		 * bTree.traverseNode(value,((Number) item.getLabel()).doubleValue()); }
		 * else { bTree.updateNominal(value, ((Number)
		 * item.getLabel()).doubleValue()); } } else { bTree = new
		 * BTreeNode(feature, value, (Number)item.getLabel());
	...
	/**
	 * @return regression model learner used in this node to compute regression
	 *         model
	 */
	public Regressor getRegressionModel() {
	...
	public String toString(int blanks) {
		...
		for (String feature : bTrees.keySet()) {
			...
			// double count = bTrees.get(feature).getLeqElements() +
			// bTrees.get(feature).getGreaterElements();
