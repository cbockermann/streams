...
public class StreamProcess extends OperatorChain {
	...
	final OutputPort dataStream = getSubprocess(0).getInnerSources()
			.createPort(DataStreamPlugin.DATA_STREAM_PORT_NAME);
	final InputPort outputStream = getSubprocess(0).getInnerSinks().createPort(
	...
	public void doWork() throws OperatorException {
		...
		for (Operator op : nested) {
			...
			/*
			 * if( op instanceof DataStreamOperator ){ log.info(
			 * "Resetting stream-operator {}", op ); ((DataStreamOperator)
			 * op).reset(); }
			 */
		}
