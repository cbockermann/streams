...
public class Partitioner {
	...
	protected void initOuputDirectory(File outputDirectory) throws Exception {
		...
		if (outputDirectory.isFile())
			throw new Exception("Output directory '"
					+ outputDirectory.getAbsolutePath() + "' is a file!");

		if (!outputDirectory.isDirectory())
			outputDirectory.mkdirs();

		if (!outputDirectory.isDirectory())
			throw new Exception("Failed to create outputDirectory '"
					+ outputDirectory.getAbsolutePath() + "'!");

	}

	public List<File> shuffledPartitions(int blockSize, int numberOfPartitions,
			int limit, URL url, File outputDirectory) throws Exception {
		List<File> partitions = new ArrayList<File>(numberOfPartitions);
		List<BlockWriter> writers = new ArrayList<BlockWriter>(
				numberOfPartitions);
		...
		PrintStream stats = new PrintStream(new FileOutputStream(
		...
		for (int i = 0; i < numberOfPartitions; i++) {
			File partFile = new File(outputDirectory.getAbsolutePath()
					+ File.separator + name + ".part" + fmt.format(i));
		...
		while (line != null && !writers.isEmpty() && count < limit) {
			...
			if (w.size() >= blockSize) {
				log.info(
						"Block writer {} is full, closing file {} and removing the writer",
						idx, w.getFile());
				w.close();
				writers.remove(w);
			}
			...
			if (count % 10000 == 0) {
				log.info("Processed {} examples,  {} completed",
						fmt.format(count.doubleValue() / total.doubleValue()),
						count);
				// ExperimentLog.log( "Processed {} examples,  {} completed",
				// fmt.format( count.doubleValue() / total.doubleValue() ),
				// count );
			}
		...
		if (!writers.isEmpty()) {
			log.warn(
					"Not all buckets have been fully filled with data, closing remaining {} blocks",
					writers.size());
	...
	public List<File> createPartitions(int blockSize, int numberOfPartitions,
			int limit, URL url, File outputDirectory) throws Exception {
		List<File> partitions = new ArrayList<File>(numberOfPartitions);
		List<BlockWriter> writers = new ArrayList<BlockWriter>(
				numberOfPartitions);
