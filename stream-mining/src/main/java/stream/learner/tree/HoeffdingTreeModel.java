...
import stream.model.DescriptionModel;
import stream.model.PredictionModel;

/**
 * Represents a Hoeffding Tree. The type of the nodes in this tree is
 * {@link HoeffdingTreeNode}
 * 
 * @author Tobias Beckers
 * 
 */
public class HoeffdingTreeModel extends PredictionModel<String> implements
	...
	/**
	 * Creates a new Hoeffding Tree containing only an empty root with an
	 * initial label.<br/>
	 * Defines a type for each feature.
	 * 
	 * @param featureTypes
	 *            a declaration of types for all features
	 * @param label
	 *            an initial Label for the root
	 */
	public HoeffdingTreeModel(String name, Map<String, Class<?>> featureTypes,
			Serializable label) {
		super(name);
	 ...
	 * 
	 * @param feature
	 *            a feature which type is requested
	 ...
	 * 
	 * @param example
	 *            an {@link Example} for which a prediction should be made
	 * @return the predicted class label for the specified example
	 */
	@Override
	public String predict(Data example) {
		return this.getLeaf(example).getLabel().toString();
	}
		...
		while (!currentNode.isLeaf()) {
			...
			if (clazz == Double.class)
				currentNode = currentNode.getchild(
						example.get(currentNode.getFeature()), Double.class);
			else
				currentNode = currentNode.getchild(
						example.get(currentNode.getFeature()), String.class);
		}
		return currentNode;
	}
