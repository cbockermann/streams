...
/**
 * <p>
 * {@link Model}-part of the implementation of the CountSketch algorithm from
...
public class CountSketchModel<T> implements StaticTopKModel<T> {
	...
	/**
	 * <p>
	 * As CountSketch also provides top-k estimation a parameter k can be
	 * provided. If <= 0 top-k overhead is disabled.
	 * </p>
	 */
	protected int k;
	...
	/*
	 * Have to operate with collections since we cannot instantiate generic
	 * arrays
	 */
	/**
	 * <p>
	...
	public CountSketchModel(int domain, int nrOfHashFunctions, int nrOfbuckets,
			int k, HashFunctionFactory<T> hashFactory) {
	...
	protected void initializeHashes(int domain, int nrOfHashFunctions,
			int nrOfbuckets, HashFunctionFactory<T> factory) {
	 ...
	 * 
	 * @param item
	 *            the item which frequency shall be estimated
	...
	CountEntry<T> getItemWithLowestCount() {
		return Collections.min(topItems.values(),
				new Comparator<CountEntry<T>>() {
	 ...
	 * 
	 * @param item
	 *            the item to insert into the data structure
	 */
