...
public class LearnerUtils extends DataUtils {
	...
	public static boolean isLabel(String name) {
		return name.startsWith(Data.ANNOTATION_PREFIX + "label")
				|| name.equals("label") || name.startsWith("class")
				|| name.startsWith("_class");
	}
	...
	public static Set<String> getAttributes(Data item) {
		Set<String> set = new LinkedHashSet<String>();
		for (String key : item.keySet())
			if (!isLabel(key) && !isAnnotation(key))
				set.add(key);
		return set;
	}
	...
	public static Set<String> getNumericAttributes(Data item) {
		Set<String> set = new LinkedHashSet<String>();
		for (String key : item.keySet())
			if (!isLabel(key) && isNumerical(key, item))
				set.add(key);
		return set;
	}
	...
	public static String getMaximumKey(Map<String, Double> input) {
		...
		for (String feature : input.keySet()) {
			if (bestValue == null
					|| bestValue.compareTo(input.get(feature)) < 0) {
				bestFeature = feature;
