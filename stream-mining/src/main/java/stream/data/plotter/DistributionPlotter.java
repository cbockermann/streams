 ...
 * 
 */
public class DistributionPlotter extends AbstractPlotter implements
		DataStreamListener, Plotter {
		...
		for (String key : data.keySet()) {
			...
			if (key.equals(label)) {
				...
				if (data.get(key).getClass().equals(Double.class)) {
					...
					if (dist == null) {
						dist = new NumericalDistributionModel("dist(" + key
								+ ")", bins, 0.1);
	...
	public void updateChart() {
		...
		try {
			...
			// plot.getDomainAxis().setLabelFont( new Font( Font.SERIF,
			// Font.BOLD, 14 ) );
			// plot.getRangeAxis().setLabelFont( new Font( Font.SERIF,
			// Font.BOLD, 14 ) );

			for (int i = 0; i < dataset.getSeriesCount(); i++) {
				plot.getRendererForDataset(dataset).setSeriesPaint(i,
						colors[i % colors.length].brighter());
			}

			File tmp = new File(this.outputFile.getAbsolutePath() + ".tmp"
					+ System.currentTimeMillis());
			log.debug("Writing chart to file {}", outputFile);

			ChartUtilities.saveChartAsPNG(tmp, chart, getSize().width,
					getSize().height);
			tmp.renameTo(outputFile);

			JFreeChart cat = ChartFactory.createBarChart(this.getTitle(),
					"Values", "Frequency", category, PlotOrientation.VERTICAL,
					true, false, false);
			File target = new File(outputFile.getParentFile().getAbsolutePath()
					+ File.separator + "nominal-" + outputFile.getName());
			ChartUtilities.saveChartAsPNG(tmp, cat, getSize().width,
					getSize().height);
			tmp.renameTo(target);
	...
	public String createHtmlEmbedding() {
		...
		b.append("     <img src=\"" + outputFile.getName()
