...
/**
 * 
 * This implementation computes approximate phi-quantiles that differ at most
 * epsilon*N ranks from the element that is the real quantile. epsilon is known
 * a priori and affects N, which is the minimum size of the set of observations.
 * To get a hunch how many examples are need I'm providing some examples:
 * <ul>
 ...
 * </ul>
 * <b>You can use this class to estimate quantiles, as long as the distribution
 * of observations doesn't have a heavy tail.</b>
 * 
...
public class SimpleQuantiles extends DeterministicQuantileEstimator {
	 ...
	 * 
	 * @param epsilon
	 *            This quantile estimator determines a phi-quantile that differs
	 *            at most epsilon*N positions from the real quantile
	 */
	...
	public QuantileModel getModel() {
		final SimpleQuantiles sq = this;
		return new QuantileModel("quantiles") {

			/** The unique class ID */
			private static final long serialVersionUID = 1L;

			/**
			 * @see stream.quantiles.QuantileModel#getQuantile(java.lang.Double)
			 */
			@Override
			public Double getQuantile(Double phi) {
				return sq.getQuantile(phi);
			}
		};
	}
	 ...
	 * 
	 * @param phi
	 *            - a {@link Double} value between 0 and 1
	 * @return {@link Double} value of an appropriate quantile or
	 *         {@link Double#NaN} if the buffer isn't ready yet.
	 */
	public Double getQuantile(double phi) {
	 ...
	 * During that initial phase every call of {@link #getQuantile(double)} will
	 * return {@link Double#NaN}. You can call this method to check whether the
	 * initial phase has ended or not.
	 * 
	...
	/**
	 * The size of the buffer depends on the given epsilon. This method computes
	 * an appropriate size for the buffer.
	 * 
	 * @return an <code>int</code> value that serves as the maximum length for
	 *         the buffer
	 */
	...
	/**
	 * Because the quantile lays in an interval there have to be bounds on the
	 * ranks. This method computes the lower one.
	 * 
	 * @param phi
	 *            The specific quantile.
	...
	private Integer computeLowerBound(double phi) {
		...
		double gaussianPQuantile = t
				- (2.515517 + 0.802853 * t + 0.010328 * Math.pow(t, 2))
				/ (1 + 1.432788 * t + 0.189269 * Math.pow(t, 2) + 0.001308 * Math
						.pow(t, 3));
		Double bound = 1d;
		bound = Math.ceil(this.bufferSize * phi - gaussianPQuantile
				* Math.sqrt(this.bufferSize * phi * (1 - phi)));
		if (bound <= 0) {
			bound = 1d;
		}
	...
	/**
	 * Because the quantile lays in an interval there have to be bounds on the
	 * rank. This method computes the upper one.
	 * 
	 * @param phi
	 *            The specific quantile.
