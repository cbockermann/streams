...
/**
 * <p>
 * This class extends the NaiveBayes implementation but overrides the methods
 * which are responsible for creating the distribution estimators. This
 * implementation of NaiveBayes provides several strategies for estimating an
 * attribute's distribution, i.e. different approximative counters for nominal
 * attributes, quantile estimators for numerical attributes, etc.
 * </p>
 * 
...
public class StickySamplingBayes extends NaiveBayes {
	 ...
	 *            the probabilityOfFailure to set
	 */
	...
	public NominalDistribution<String> createNominalDistribution() {
		...
		if (getSupport() != null)
			...
			log.warn("Parameter 'support' not specified, using default: {}",
					sup);
		...
		if (getProbabilityOfFailure() != null) {
			...
			log.warn(
					"Parameter 'probabilityOfFailure' not specified, using default: {}",
					pof);

		StickySampling<String> stickySampling = new StickySampling<String>(sup,
				err, pof);
		return new NominalDistribution<String>(stickySampling);
	}

	/**
	 * @see stream.classifier.NaiveBayes#createNumericalDistribution()
	 */
	@Override
