...
public class StreamGenerator extends Thread {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			...
			System.out
					.println("    stream.generator.Generator  stream-config.xml output-file.csv\n");
			return;
		}

		File file = new File(args[0]);
		File out = new File(args[1]);
		LabeledGaussianStream stream = LabeledGaussianStream
				.createFromFile(file);
		...
		// PrintStream desc = new PrintStream( new FileOutputStream( new File(
		// out.getAbsolutePath().replaceAll( "csv$", "html" ) ) ) );
