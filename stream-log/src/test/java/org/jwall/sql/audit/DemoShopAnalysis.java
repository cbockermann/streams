	...
	public static void main(String[] args) throws Exception {
		...
		sqlParser.addMapper(new Mapper<String, String>() {
			...
			public String map(String input) throws Exception {
				...
				if (input.indexOf("AND SLEEP(5)") >= 0) {
					return input.replace("AND SLEEP(5)",
							"AND myDummySleep > SLEEP(5)");
				}
		...
		while (item != null) {
			...
			if (("" + item.get("sql:query")).indexOf("SLEEP") > 0) {
				log.info("Item has valid SQL: {} => {}", item.get("sql:ok"),
						item.get("sql:query"));
				log.info("Item has query-tree: {}", item.get("@tree:sql:query"));
			}
			boolean validMySQLSyntax = "true".equals(item.get("sql:ok"));
			boolean sqlMapAttack = ("" + item.get("REQUEST_HEADERS:User-Agent"))
					.indexOf("sqlmap") >= 0;
			sqlMapAttack = ("" + item.get("REQUEST_HEADERS:User-Agent"))
					.indexOf("Java/1.6.0_26") < 0;
			boolean jsqlParse = item.get("@tree:sql:query") != null;

			String query = item.get("sql:query").toString().trim();
			if (query
					.matches("^SELECT id,name,description,price FROM products WHERE id = \\d+$")) {
			...
			counts.add("validMysql: " + validMySQLSyntax + "  sqlmapAttack: "
					+ sqlMapAttack + "  jsqlParse: " + jsqlParse);
		...
		// out.close();
		writer.finish();
		log.info("{} statements have been read, {} unique queries.", count,
				stmts.size());
