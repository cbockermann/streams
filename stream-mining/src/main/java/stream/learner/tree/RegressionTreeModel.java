 ...
 * 
 */
public class RegressionTreeModel extends PredictionModel<Double> {
	/** The unique class ID */
	 ...
	 */
	public RegressionTreeModel(String name, String linearRegressionClass,
			Map<String, ?> parameters) throws Exception {
		super(name);
		Regressor regression = null;
		ObjectFactory.newInstance();
		Class<?> clazz = Class.forName(linearRegressionClass);
		regression = (Regressor) clazz.newInstance();
	...
	public Double predict(Data item) {
		...
		if (leaf != null) {
			Double prediction = leaf.getRegressionModel().classify(item);
			return prediction;
		}
		return Double.NaN;
	}
