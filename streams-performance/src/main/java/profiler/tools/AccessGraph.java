    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int j = 0; j < processes.getLength(); j++) {
            ...
            for (int i = 0; i < ps.size(); i++) {
                ...
                p.println("\\node[rotate=-30,anchor=west,scale=0.65] at " + new Point(i * stretch, -1 * scaleY - 0.25)
                        + " {\\ttfamily " + simple + "};");

                double x = i;

                List<Element> reads = XMLUtils.getElementsByName(e, "read");
                for (int r = 0; r < reads.size(); r++) {
                    Element read = (Element) reads.get(r);
            ...
            for (String k : ks) {
                ...
                p.println("\\node[scale=1.0,anchor=east,scale=0.5] at " + new Point(-1, idx * scaleY)
