...
/**
 * <p>
 * This class implements a test-and-train strategy for data streams. Elements
 * arriving at instances of this class will be used for testing and after that
 * will be provided to all the registered learners for training.
 * </p>
 * 
...
public class TestAndTrain<L extends Learner<?>> implements DataStreamListener {
	...
	public TestAndTrain(Mapper<Data, Data> dataMapper,
		...
		if (test instanceof Mapper) {
			//
			// TODO: Put this into a preprocessing chain for the test-and-train
			// stuff...
			//
	...
	public Statistics test(Data data) {
		...
		log.debug("Data item count is {}, running evaluation on data: {}",
				tests, data);
		...
		for (StatisticsListener l : this.performanceListener) {
			...
			// ) );
			// l.dataArrived( totalError.divideBy( "Events" ).setName(
			// "Error averaged over #events" ).setKey( "average-model-error" )
			// );
	...
	public Map<String, String> getProperties() {
		...
		for (String key : test.getLearnerCollection().keySet())
			p.put("test.learner['" + key + "']", test.getLearnerCollection()
					.get(key).toString());
	...
	public String createLearnerTableHtml() {
		...
		s.append("<tr><td><code>Groud Truth</code></td><td><code>"
				+ test.getBaselineLearner().toString() + "</td></code></tr>\n");

		for (String key : test.getLearnerCollection().keySet()) {
			s.append("<tr>");
			s.append("<td><code>" + key + "</code></td>");
			s.append("<td><code>"
					+ test.getLearnerCollection().get(key).toString()
							.replace("{", "</code><br/><code> {")
					+ "</code></td>");
			s.append("</tr>");
		}
