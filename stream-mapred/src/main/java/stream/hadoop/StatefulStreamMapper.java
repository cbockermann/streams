...
/**
 * <p>
 * This class implements a simple mapper that will process a block of data
 * items. In addition to that is provides methods for storing temporal state
 * data in a local memcached instance.
 * </p>
 * 
 * @author Christian Bockermann
 * 
 */
public abstract class StatefulStreamMapper extends AbstractStreamMapper {
	...
	public StatefulStreamMapper() {
		if (System.getProperty("memcache.address") != null) {
			try {
				memcache = new MemcachedClient(AddrUtil.getAddresses(System
						.getProperty("memcache.address")));
			} catch (Exception e) {
				e.printStackTrace();
				if (memcache != null) {
	...
	public boolean remember(String key, Serializable object) {
		...
		if (memcache != null) {
			...
			try {
				...
				log.error("Failed to store value for key {}: {}", key,
						e.getMessage());
			}
		}
	...
	public void setExpireTime(Integer expireTime) {
		if (expireTime < 0) {
			log.warn("Negative expire-time is not supported! Sticking to {}",
					this.expireTime);
			return;
		}
