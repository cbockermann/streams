...
/**
 * <p>
 * This very simple class checks for label attributes and prediction attributes
 * and records all their differences.
 * </p>
 * 
	...
	public Data process(Data data) {
		...
		for (String key : data.keySet()) {
			if (key.startsWith(Prediction.PREDICTION_PREFIX)) {
				...
				String errorKey = Data.ANNOTATION_PREFIX + "predictionError["
						+ learnerName + "," + labelKey + "]";
				Serializable prediction = data.get(key);

				Double err = 0.0d;

				if (LearnerUtils.isNumerical(labelValue)
						&& LearnerUtils.isNumerical(prediction))
