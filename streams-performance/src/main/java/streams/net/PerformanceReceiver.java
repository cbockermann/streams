...
//    static MultiSet<String> updateCount = new MultiSet<>();
    ...
    /**
     * Run method for the performance receiver thread. It starts the updater and
     * receiver daemons.
     */
    public void run() {
        try {
    ...
    /**
     * Initialize updater object. This method can be overwritten in the way as in {@link
     * MergedPerformanceReceiver}.
     */
    protected void initUpdater() {
        // add updater
        log.info("Initialize Updater as updater.");
        Updater updater = new Updater(updates, performanceTrees);
    ...
    public static class Receiver extends Thread {
        final Socket socket;
        final Codec<Data> codec = new DefaultCodec<Data>();
        ...
        public void run() {
            try {
                        ...
                        if (perfs != null && perfs.getClass().isArray()
                            ...
                            if (perfs != null && perfs instanceof ProcessorStatistics) {
                                ...
                                for (int i = 0; i < stats.length; i++) {
                                    updates.add(new Update(id.toString() + "/processor:" + i + ":" + stats[i].className,
                                            stats[i]));
                                }
                            }
                        }
                ...
                log.error("Receiver thread has been stopped or " + "was interrupted by some exception:" + e);
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        if (args.length == 1) {
            try {
                ...
                log.error("You can only define port number as a parameter. " + " Using default: 6001." + args[0]);
            }
        }
