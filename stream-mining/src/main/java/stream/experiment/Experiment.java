...
public class Experiment {
	...
	TestAndTrain<Learner<?>> evaluation;
	...
	protected void init(Document doc) throws Exception {
		...
		this.experimentFile = new File(outputDirectory.getAbsolutePath()
				+ File.separator + "experiment.xml");
		Map<String, String> settings = ExperimentFactory.findGlobalSettings(doc
				.getDocumentElement());
		...
		List<Element> dataSourceElements = ExperimentFactory
				.findDataSourceNodes(doc);
		...
		try {
			String dsDescription = null; // dataSource.retrieveDescription(
											// ".en.xml");
			...
			try {
				...
				Document xmlDesc = dbf.newDocumentBuilder().parse(
						dataSource.getUrl() + ".en.xml");
				log.info("Imported description from XML remote source");
				dataSourceElements.get(0).appendChild(
						doc.importNode(xmlDesc.getDocumentElement(), true));
			} catch (Exception e) {
				// log.info( "DataSource description:\n{}\n", dsDescription );
				// dataSources.get(0).setTextContent( dsDescription.replace(
		...
		tf = TransformerFactory
				...
				.newTransformer(
						new StreamSource(
								Experiment.class
										.getResourceAsStream("/templates/experiment-report.xsl")));
		result = new StreamResult(new FileOutputStream(new File(this
		...
		ReportProcessor.copyResource("/templates/style.css",
	...
	public void init() throws Exception {
		...
		File target = new File(outputDirectory.getAbsolutePath()
				+ File.separator + experimentFile.getName());
		if (!target.isFile()) {
		...
		//
		// QuantileEstimatorTest eval = new QuantileEstimatorTest( new
		// ExactQuantiles() );
		if (evaluation == null) {
			log.info("No evaluation exists, creating empty evaluation");
			AbstractTest<Learner<?>> evaluator = null;
			evaluation = new TestAndTrain<Learner<?>>(null, evaluator);
		}

		log.info("Setting up plotter..., learner names: {}",
				evaluation.getLearnerNames());

		StreamPlotter memoryPlot = new StreamPlotter("Events",
				evaluation.getLearnerNames(), new File(this
						.getOutputDirectory().getAbsolutePath()
						+ File.separator + "memory.png"));
		memoryPlot.setTitle("Memory Usage");
		if (updatePlots) {
			plots.add(memoryPlot);
			evaluation.addMemoryListener(memoryPlot);
		}
		File memoryStats = new File(getOutputDirectory().getAbsolutePath()
				+ File.separator + "memory.dat");
		...
		StreamPlotter errorPlot = new StreamPlotter("Events", errors, new File(
				this.getOutputDirectory().getAbsolutePath() + File.separator
						+ "model-error.png"));
		...
		File modelError = new File(getOutputDirectory().getAbsolutePath()
				+ File.separator + "model-error.dat");
		...
		evaluation.addPerformanceListener(new WindowedStatisticsListener(
				new StatisticsStreamWriter(modelError), windowSize, true)); // new
																			// File(
																			...
																			// )
																			// );
		evaluation.addPerformanceListener(new MultiStatisticsWriter(
				getOutputDirectory()));
		...
		StreamPlotter vmMemoryPlot = new StreamPlotter(new File(this
				.getOutputDirectory().getAbsolutePath()
				+ File.separator
				+ "vm-memory.log"));
	...
	public void run() throws Exception {
		...
		if (stream == null) {
			log.info("Data stream not yet initialized, opening dataSource {}",
					dataSource);
		...
		//
		// The experiment loop: read from the stream and test/train on each
		// element
		//
		...
		log.info(
				"Skipped first {} data stream items, due to experiment range settings",
				i);

		log.info("Starting GNUplot runner...");
		GnuplotRunner gnuplot = new GnuplotRunner(getOutputDirectory(),
				1000L * plotInterval);
		if (this.getGnuplot())
			gnuplot.start();

		/*
		 * StreamSummarizer summarizer = new StreamSummarizer(); // new File( //
		 * getOutputDirectory().getAbsolutePath() // + // File.separator // + //
		 * "stream-statistics.html" // ) );
		 * this.outputElements.put("STREAM_SUMMARY", summarizer);
		 * preEvaluation.add(summarizer);
		 */

		outputElements.put("CONFUSION_MATRIX",
				new PredictionResults(evaluation));
		...
		while (datum != null && ++i < limit) {
			try {
				...
				if (i % mark == 0) {
					...
					log.info(
							"Processed {} elements, currently using {}k of memory.",
							i, mem1 / 1024);
					updateHtml();
				}
		...
		log.info("{} data items processed ({} items/sec).", i,
				i / seconds.doubleValue());
	}
		...
		try {
			...
			for (String element : outputElements.keySet()) {
				if (result.indexOf(element) >= 0) {
					log.debug("Found element {} at position: {}", element,
							result.indexOf(element));
				} else
					log.debug("Did not find element {} in HTML result!",
							element);
				result = result.replaceAll(element, outputElements.get(element)
						.toHtml());
			}

			PrintStream out = new PrintStream(new FileOutputStream(this
					.getOutputDirectory().getAbsolutePath()
					+ File.separator
					+ "index.html"));
			out.println(result);
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * 
		 * if( args.length == 0 ){ log.error( "Experiment-file required!" );
		 * return; }
		 * 
		 * File file = new File( args[0] );
		 * 
		 * if( !file.canRead() ){ log.error( "Cannot read experiment-file {}!",
		 * file ); return; }
		 * 
		 * try { log.info( "Running experiment from file {}", file );
		 * 
		 * if( ! file.isAbsolute() ){ file = new File( (new File( "."
		 * ).getAbsolutePath() + File.separator + file.getPath() ) ); log.info(
		 * "File is a relative file, using: {}", file ); } Experiment e =
		 * ExperimentFactory.parseExperiment( file ); e.init(); e.run(); } catch
		 * (Exception e) { log.error( "Execution of experiment failed: {}", e );
		 * e.printStackTrace(); }
		 */
	}
}