...
public class ExperimentFactory {
	...
	public static Learner<?> create(String className,
		...
		if (!Learner.class.isAssignableFrom(clazz))
			throw new Exception("Class '" + className
					+ "' does not implement the Learner interface!");
	...
	public static Map<String, Map<String, String>> findLearnerConfigs(URL url)
			throws Exception {
	...
	public static Experiment parseExperiment(File file) throws Exception {
		...
		Map<String, Map<String, String>> streams = findElement("stream",
				doc.getDocumentElement());
		if (!streams.isEmpty()) {
			Map<String, String> map = streams.values().iterator().next();
			DataSource ds = new DataSource(map.get("name") + "",
					map.get("url"), map.get("class"));
			e.setDataSource(ds);
		}
	...
	public static Map<String, String> findGlobalSettings(Node node)
			throws Exception {
		Map<String, String> found = new LinkedHashMap<String, String>();

		if (node.getNodeName().equalsIgnoreCase("experiment")
			...
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				if (attribute.getNodeType() == Node.ATTRIBUTE_NODE) {
					log.info("Adding experiment property {} = {}",
							attribute.getNodeName(), attribute.getNodeValue());
					globalSettings.put(attribute.getNodeName(),
	...
	public static Map<String, Map<String, String>> findElements(String element,
	...
	public static Map<String, DataSource> findDataSources(Node node)
			throws Exception {
		...
		for (String key : ds.keySet()) {
			DataSource d = new DataSource(key, ds.get(key).get("url"), ds.get(
					key).get("class"));
			Map<String, String> params = ObjectFactory.newInstance()
					.getAttributes(node);
	...
	public static List<Element> findDataSourceNodes(Document doc)
			throws Exception {
	...
	public static Map<String, Map<String, String>> findLearner(Node node)
			throws Exception {
		...
		if (node.getNodeName().equalsIgnoreCase("learner")) {
			...
			if (name == null)
				throw new Exception(
						"Missing 'name' attribute for learner node!");
	...
	public static Map<String, Learner<?>> createLearner(URL url)
			throws Exception {
		Map<String, Learner<?>> result = new HashMap<String, Learner<?>>();

		Map<String, Map<String, String>> configs = ExperimentFactory
				.findLearnerConfigs(url);
		for (String key : configs.keySet()) {
			...
			// ) );
			Learner<?> l = ExperimentFactory.create(parameters.get("class"),
					parameters);
	...
	public static Map<String, Learner<?>> createLearner(Document doc)
			throws Exception {
		Map<String, Learner<?>> result = new HashMap<String, Learner<?>>();

		Map<String, Map<String, String>> configs = ExperimentFactory
				.findLearner(doc.getDocumentElement());
		for (String key : configs.keySet()) {
			Map<String, String> parameters = configs.get(key);
			log.info("Learner '{}' has config: {}", key, configs.get(key));
			// log.info( "   class of Learner is '{}'", parameters.get( "class"
			...
			if (parameters.get("class") != null && result.containsKey(key)) {
				...
				throw new Exception(
						"Multiple learners defined with the same name '" + key
								+ "'!");
			}
			Learner<?> l = ExperimentFactory.create(parameters.get("class"),
					parameters);
	...
	public static Learner<?> createLearner(Node node) throws Exception {
		...
		if (node.getNodeType() == Node.ELEMENT_NODE
				&& node.getNodeName().equalsIgnoreCase("learner")) {
			...
			Learner<?> learner = ExperimentFactory.create(config.get("class"),
					config);
	...
	public static Map<String, Map<String, String>> findElement(
			String elementName, Node node) throws Exception {
		Map<String, Map<String, String>> found = new LinkedHashMap<String, Map<String, String>>();
		...
		if (node.getNodeName().equalsIgnoreCase(elementName)) {
			...
			NodeList list = node.getChildNodes();
			for (int i = 0; i < list.getLength(); i++) {
				Map<String, Map<String, String>> cfg = findElement(elementName,
						list.item(i));
				if (cfg != null)
					found.putAll(cfg);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		Map<String, Map<String, String>> streams = ExperimentFactory
				.findElements("stream", url);
	...
	public static Map<String, DataStream> createDataStreams(Document doc)
			throws Exception {
		Map<String, DataStream> ds = new LinkedHashMap<String, DataStream>();

		Map<String, Map<String, String>> streams = ExperimentFactory
				.findElement("stream", doc.getDocumentElement());
		for (String key : streams.keySet()) {
	...
	public static TestAndTrain<Learner<?>> findEvaluation(Document doc)
			throws Exception {
		...
		Map<String, Map<String, String>> baseConfig = ExperimentFactory
				.findElement("Baseline", t3);
		if (baseConfig.isEmpty() || baseConfig.size() > 1) {
			baselineProvided = false;
			baselineConfig.put("class", "stream.learner.LabelPredictor");
			log.info("No baseline learner found, using preparing default: {}",
					baselineConfig);
		} else {
		...
		baselineLearner = ExperimentFactory.create(baselineConfig.get("class"),
				baselineConfig);
		...
		for (Element e : learnerNodes) {
			...
			if (name == null || name.trim().isEmpty()) {
				name = getNextIdentifier(learnerInstance.getClass()
						.getSimpleName());
			...
			Map<String, String> conf = ParameterInjection
					.extract(learnerInstance);
		...
		AbstractTest<Learner<?>> test = (AbstractTest<Learner<?>>) con
				.newInstance();

		if (test.getBaselineLearner() == null || baselineProvided) {
			log.info("Setting baselineLearner, (provided: {})",
					baselineProvided);
