 ...
 * 
 */
public class NaiveBayes extends AbstractClassifier<String> {
	/** The unique class ID */
	 ...
	 */
	public NaiveBayes(String labelAttribute) {
		this();
	...
	public String classify(Data item) {
		...
		if (log.isDebugEnabled()) {
			log.debug("labelAttribute is {}, data-item is: {}", labelAttribute,
					item);
			log.debug("Predicting class {}, label is: {}, confidence-gap: "
					+ confidence + " wop=" + wop, maxClass,
					item.get(labelAttribute));
		}
		return maxClass;
	}
	...
	public void learn(Data item) {
		...
		for (String attribute : item.keySet()) {
			...
			if (item.get(attribute) == null) {
				log.debug("Skipping empty attribute {}", attribute);
				continue;
			}
	...
	public PredictionModel<String> getModel() {
		return new PredictionModel<String>(getId()) {
			/** The unique class ID */
			private static final long serialVersionUID = -3919019323726881425L;
