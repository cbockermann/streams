...
import stream.quantiles.QuantileModel;
import stream.quantiles.WindowSketchQuantiles;

/**
 * This class is out to realize a stream version of the rss technique presented
 * by Anna Gilbert, Yannis Kotidis, S. Muthukrishanan, and Matrin Strauss in the
 * paper "How to summarize the Universe"
 * 
...
public class RandomSubsetSums extends ProbabilisticQuantileEstimator {
	 ...
	 *            error-probability
	 * @param maxValue
	 *            the maximum value the will be handled by this algorithm (|U|)
	 */
	...
	/**
	 * create a list of dyadic intervals that will, added together, describe the
	 * rank
	 * 
	 * @param rank
	 */
	...
	/**
	 * This method deploys the rss way to find a quantile. Its based on finding
	 * the sums of different intervals. To have a correct value we must consider
	 * all {@link Bucket} in the sliding window.
	 */
	@Override
	public QuantileModel getModel() {
		return new QuantileModel(getId()) {
			...
			public Double getQuantile(Double phi) {
				...
				int wantedRank = (int) ((float) overallBucketCount
						* phi.floatValue() - (float) overallBucketCount
						* epsilon);

				// this loop creates dyadic intervals 0..i and lets all buckets
				// evaluate them.
				// then it tests if we have reatched the wanted rank with our
				// sum.
				for (int i = 0; i < maxValue; i++) {
					LinkedList<Interval> intervals = RandomSubsetSums.this
							.collectNeededIntervalls(i);
					double intervalSum = 0;
					for (Bucket bucket : new LinkedList<Bucket>(buckets)) {
						intervalSum += Math.abs(bucket
	 ...
	 * test function
	 */
	public static void main(String args[]) { // <------ need since still in
												// development
		...
		for (int k = 0; k < 5; k++) {
			...
			for (int i = 0; i < 1000; i++) {
				int nextElement = random.nextInt(10) + 5; // generate pw-length
															// between 5 and 15
			...
			// System.out.println("rss quantile = " +
			// rss.getModel().describe(0.5) );
			// System.out.println("exact quantile = " +
			// exact.getQuantile(0.5f));
			System.out.println("WindowSketchQuantiles = "
					+ wsq.getModel().getQuantile(0.5));
