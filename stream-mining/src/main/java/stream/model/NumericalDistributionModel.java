 ...
 * 
 */
public class NumericalDistributionModel implements
		SelectiveDescriptionModel<Double, Double>, Distribution<Double> {

	/** The unique class ID */
	private static final long serialVersionUID = -4642672370564928117L;
	static Logger log = LoggerFactory
			.getLogger(NumericalDistributionModel.class);
	...
	/**
	 * Creates a new distribution model. The specified parameter determines the
	 * interval of the bins, that are created.
	 * 
	 * @param interval
	 */
	public NumericalDistributionModel(String name, Integer binCount, Double span) {
		this.name = name;
	...
	public Map<Double, Double> getHistogram() {
		...
		for (int i = 0; i < histogram.length; i++)
			d.put(histogram[i].doubleValue(), histogram[i].doubleValue()
					/ total);
