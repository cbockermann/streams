...
/**
 * <p>
 * {@link Model}-part of the implementation of the CountMinSketch algorithm from
 * the paper 'An improved data stream summary: the count-min sketch and its
 * applications' written by 'Cormode, G. and Muthukrishnan, S. (2003)'.<br />
 * </p>
 * 
...
public class CountMinSketchModel<T> extends CountSketchModel<T> {
	...
	public CountMinSketchModel(int domain, int nrOfHashFunctions,
			int nrOfbuckets, int k) {
		super(domain, nrOfHashFunctions, nrOfbuckets, k,
				new SimpleHashFactory<T>());
	}

	/**
	 * <p>
	 * We only need the h-hash functions since the CountMinSketch algorithm
	 * always increments the values in the data.
	 * </p>
	 * 
	 * @param domain
	 *            the (estim.) domain, i.e. how many different items are
	 *            expected
	 * @param nrOfHashFunctions
	 *            the number of hashfunctions which determine a bucket
	 * @param nrOfbuckets
	 *            the number of buckets where a counter will be maintained
	 */
	protected void initializeHashes(int domain, int nrOfHashFunctions,
			int nrOfbuckets) {
	...
	/**
	 * <p>
	 * Updating the data. For each hashfunction the corresponding bucket will be
	 * incremented by one.
	 * </p>
	 */
	@Override
	...
	/**
	 * <p>
	 * Estimates the frequency by returning the smallest frequency value for
	 * each hasfunction index.
	 * </p>
	 */
	@Override
