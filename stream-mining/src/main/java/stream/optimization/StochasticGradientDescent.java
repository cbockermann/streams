...
public class StochasticGradientDescent extends AbstractClassifier<Double>
		implements Measurable
// implements Learner<Data,PredictionModel<Data,Double>>,
// PredictionModel<Data,Double>, Measurable
{
	/** The unique class ID */
	...
	int gradVarMaxSamples = 100; // Number of samples to be used for estimating
	...
	public double getByteSize() {
		//
		// 32 bytes is from t, b, sum_etha and D as each is stored as 64bit
		// double
		//
		double bs = 32.0d;
		if (gt != null)
	...
	protected double etha() {
		...
		// return obj.getRadius() / Math.sqrt( obj.getGradientNormVariance() *
		// 1000.0d );
	...
	public void learn(Data example) {
		...
		if (this.gradVarSamples < this.gradVarMaxSamples) {
			...
			++this.gradVarSamples;
			// System.out.println("SGD Init: obj.G = " +
			// Math.sqrt(obj.getGradientNormVariance()));
			return;
		}
		...
		/*
		 * gt = obj.subgradient( w, x_i, label ); // w = w + (-1.0 * eta) gt //
		 * w = w.add( (-1.0 * eta), gt );
		 */
	 ...
	 */
	@Override
	public Double classify(Data example) {
		InputVector x_i;
