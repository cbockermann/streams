...
public class MysqlSessionTrackerTest {
	final static Logger log = LoggerFactory
			.getLogger(MysqlSessionTrackerTest.class);
	...
	public static void main(String[] args) throws Exception {
		...
		dsp.addDataProcessor(new AbstractDataProcessor() {
			@Override
			public Data process(Data data) {
				/*
				 * for( String key : data.keySet() ){ log.info( "  {} = {}",
				 * key, data.get( key ) ); }
				 */
		...
		sqlParser.getFixes().put("information_schema.SCHEMATA#",
				"information_schema.SCHEMATA");
		sqlParser.setErrorStream(new FileOutputStream(
				"/Users/chris/sql-mining/shop-sql-parser.err"));
		dsp.addDataProcessor(sqlParser);
		dsp.addDataProcessor(new TreeEdges());

		PrintStream out = new PrintStream(new FileOutputStream(
				"test-queries2.tsv"));
		DataStreamWriter dsw = new DataStreamWriter(new FileOutputStream(
				"test-sql.tsv"), "\t");
		dsw.setKeys("mysql:timestamp,mysql:session,mysql:database,@tree:mysql:query");
		Long count = 0L;
		Data item = dsp.readNext();

		String[] outKeys = new String[] { "mysql:timestamp", "mysql:database",
				"mysql:query", "@tree:mysql:query" };
		int written = 0;
		int i = 0;
		while (item != null && i++ < 100000) {
			...
			// }
			if (item.get("mysql:query") != null
					&& "appdb".equals(item.get("mysql:database"))
					&& item.get("@tree:mysql:query") != null) {
				dsw.dataArrived(item);
				written++;
			} else {
				// log.info( "Skipping item with mysql:database={}", item.get(
				// "mysql:database" ) );
			}
			item = dsp.readNext();
		}
		log.info("{} items written", written);
		log.info("Successfully parsed {} out of {} items",
				sqlParser.getSuccessCount(), sqlParser.getSuccessCount()
						+ sqlParser.getErrorCount());
		log.info("{} items could not be parsed.", sqlParser.getErrorCount());
		log.info("   Success rate is {}", sqlParser.getSuccessCount()
				.doubleValue() / sqlParser.getTotalCount().doubleValue());
	}
}