...
public class SgdTester extends AbstractDataProcessor {
	...
	public static SgdTester computeTestError(File modelFile, URL testUrl)
			throws Exception {

		if (!modelFile.canRead()) {
			throw new Exception("Cannot open file " + modelFile
					+ " for reading!");
		}
		...
		if (models.isEmpty()) {
			throw new Exception(
					"At least one weight vector is required for testing!");
		}
		...
		while (item != null) {
			test.process(item);
			if (++count % 1000 == 0) {
				log.debug("Test error after {} tests: {}", count,
						test.getTestError());
				log.debug("The prediction accuracy thus is: {}",
						1.0d - test.getTestError());
			}
	...
	public static void main(String[] params) throws Exception {
		...
		if (args.length < 2) {
			...
			System.err
					.println("      java stream.hadoop.SgdTester WEIGHT_FILE  TEST_SET_URL");
			System.err.println();
			System.exit(0);
		}

		Double testError = computeTestError(new File(args[0]), new URL(args[1]))
				.getTestError();
