...
public class SumQuantiles extends AbstractQuantileLearner {
	...
	public static void main(String args[]) { // <------ need since still in
		...
		for (int i = 0; i < 10; i++) {
			...
			/*
			 * if((i % 1000) == 1) { double sumResult =
			 * caq.getModel().describe(phi); double exactResult =
			 * ex.getModel().describe(phi); System.out.println("SumQuantiles   "
			 * + sumResult); System.out.println("ExactQuantiles " +
			 * exactResult); System.out.println("MSE            " + (sumResult -
			 * exactResult) * (sumResult - exactResult));
		...
		double sumResult = caq.getModel().getQuantile(phi);
		double exactResult = ex.getModel().getQuantile(phi);
		System.out.println("SumQuantiles   " + sumResult);
		System.out.println("ExactQuantiles " + exactResult);
		System.out.println("MSE            " + (sumResult - exactResult)
				* (sumResult - exactResult));
		caq.printBuckets();
