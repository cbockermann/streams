 ...
 * 
 */
public class RTree extends ModelTree<RegressionTreeStatistics, Double>
	...
	public RTree(String name, TreeNode<RegressionTreeStatistics> parent) {
		super(name, parent,
				new ChernoffSplitCriterion<RegressionTreeStatistics>());
	}
	...
	public void learn(Data item) {
		...
		boolean requiresSplit = splitCriterion
				.requiresSplit(leaf.getNodeInfo());
		...
		if (requiresSplit) {
			...
			//
			ModelTree<RegressionTreeStatistics, Double> parent = (ModelTree<RegressionTreeStatistics, Double>) leaf
					.getParent();

			ModelTree<RegressionTreeStatistics, Double> replacement = new ModelTree<RegressionTreeStatistics, Double>(
					leaf.getName(), parent, splitCriterion);
			replacement.add(new ModelTree<RegressionTreeStatistics, Double>(
					leaf.getName(), null, splitCriterion));
	...
	public Double getBestSplitValue(BTreeNode btree) {
		...
		log.info("Best split value is: {}  (SDR: {})",
				splitValueFinder.getValue(), splitValueFinder.getMaximum());
	...
	public Double classify(Data item) {
		ModelTree<RegressionTreeStatistics, Double> leaf = getLeaf(item);
		return leaf.getModel().predict(item);
	}
	 ...
	 * 
	 */
	class BestSplitValueFinder implements
			Visitor<BinaryTreeNode<RegressionTreeStatistics, Double>> {
		...
		public void visit(BinaryTreeNode<RegressionTreeStatistics, Double> node) {
			...
			if (maxValue == null) {
				...
				log.info("Found initial split value: {}  (sdr: {})", maxValue,
						maxSdr);
				return;
			}
			...
			if (sdr > maxSdr) {
				maxValue = node.getValue();
				maxSdr = sdr;
				log.info("Found new best split value: {}  (sdr: {})", maxValue,
						maxSdr);
			}
		}

		public Double getStandardDeviationReduction(
				BinaryTreeNode<RegressionTreeStatistics, Double> node) {
			Double sdr = 0.0d;
	...
	public PredictionModel<Double> getModel() {
		return new PredictionModel<Double>(getId()) {
			...
			/** The unique class ID */
			private static final long serialVersionUID = -8933858882125989432L;
